...
clone(void)
{
  ...
  if((np->pgdir = copyuvm(curproc->pgdir, curproc->sz, curproc->sharedsz)) == 0){
    kfree(np->kstack);
    np->kstack = 0;
    np->state = UNUSED;
    return -1;
  ...
  *np->tf = *curproc->tf;
  ...
  if (argptr(0, &fptr, sizeof(void *)) < 0) {
    return -1;
  }
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      if(p->state == ZOMBIE){
        ...
        kfree(p->kstack);
        p->kstack = 0;
        freevm(p->pgdir, p->shared ? p->sharedsz : 0);
        p->pid = 0;
        p->parent = 0;
        p->name[0] = 0;
        p->killed = 0;
